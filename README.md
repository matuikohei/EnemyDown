...
**ミニゲームを実装した理由**
---
このミニゲーム「EnemyDown」を実装しようと思った理由は以下の通りです。
* 実践的なプログラミングスキルの習得：<br>Javaを使って、実際に動作するソフトウェアを開発する経験を積むことで、実務に役立つスキルを習得するため。
* データベースの操作スキルの習得：<br>データベースとの連携が必要となるプラグインを開発することで、MySQLやデータベース操作の実践的なスキルを身につけるため。
* ソフトウェア開発能力の習得：<br>マインクラフトのプラグイン開発を通して、設計・実装・テストのサイクルを経験することで、ソフトウェア開発についての理解を深めるため。
