...
**工夫した点**
---
* 難易度変更の実装：　ゲームをプレイする人のスキルレベルに合わせてゲームの難易度を調整できるよう、コマンド引数を活用して難易度の変更機能を実装しました。
* 制限時間の実装：　ゲームプレイ中に緊張感を感じ、ゲーム終了後には達成感を感じられるように、スケジューラを使用して、制限時間を設定しました。
* 敵の種類によって異なる点数を設定：　ゲームを戦略的にプレイできるように、敵の種類に応じて加算されるスコアが変動するように設計し、Switch文を用いて実装しました。
* 基底クラスの作成：　コードの再利用性と拡張性を高めることを目的に、基底クラス（BaseCommand）を作成しました。
* MyBatisの使用：　スコア情報の管理を行うために、MyBatisを使用してデータの登録や取得を行いました。
* JUnitの使用：　プラグインの品質向上のため、JUnitを使用して単体テストを実施しました。
